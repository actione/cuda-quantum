...
gen_cplusplus_report() {
    if $is_codecov_format; then
        llvm-cov show ${objects} -instr-profile=${repo_root}/build/coverage.profdata --ignore-filename-regex="${repo_root}/tpls/*" \
            --ignore-filename-regex="${repo_root}/build/*" --ignore-filename-regex="${repo_root}/unittests/*" -o ${repo_root}/build/ccoverage/coverage.txt 2>&1
    else
...
if $gen_py_coverage; then
    ...
    if $is_codecov_format; then
        python3 -m pytest -v python/tests/ --ignore python/tests/backends --cov=/home/.local/lib/python3.10/site-packages/cudaq --cov-report=xml:${repo_root}/build/pycoverage/coverage.xml --cov-append
    else
    ...
    for backendTest in python/tests/backends/*.py; do
        if $is_codecov_format; then
            python3 -m pytest -v $backendTest --cov=/home/.local/lib/python3.10/site-packages/cudaq --cov-report=xml:${repo_root}/build/pycoverage/coverage.xml --cov-append
        else
